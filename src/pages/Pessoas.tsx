...
export default function Pessoas() {
  ...
  }), [scopedPeople, search, campusFilter, statusFilter]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PER_PAGE));
  const paginated  = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE);
  const hasFilters = search || campusFilter !== 'Todos' || statusFilter !== 'Todos';
  ...
  const scopeLabel = useMemo(() => {
    ...
    return '';
  }, [p.isGlobalScope, user, scopedPeople, persons.length]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-8">
      ...
      {/* Filtros */}
      <div className="card-heritage p-4 flex flex-col md:flex-row items-center gap-3">
        ...
        {hasFilters && (
          <button onClick={clear} className="flex items-center gap-1.5 text-xs font-bold text-gold px-3 py-3 whitespace-nowrap">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-surface-container-high border-b border-outline-variant">
                ...
                {p.isGlobalScope && <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Campus</th>}
                <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Estado</th>
                {p.canEditMember && <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Ações</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant">
              {isLoading ? (
                ...
                <tr key={person.id} className="hover:bg-surface-container-high/40 transition-colors">
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-9 h-9 rounded-full bg-gold/10 border border-outline-variant flex items-center justify-center text-gold font-bold text-sm flex-shrink-0">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(pg => (
              <button key={pg} onClick={() => setPage(pg)}
                className={cn('w-8 h-8 flex items-center justify-center rounded-md text-xs font-bold transition-all',
                  pg === page ? 'bg-tertiary text-white' : 'text-slate-500 hover:bg-slate-100')}>
                {pg}
              </button>
            ))}
